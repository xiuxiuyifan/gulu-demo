    ...
    <div class="left">
      ...
      >
        ...
        <template v-if="loadItem.name===item.name">
          <g-icon
            class="down-icon lodding-icon"
            icon="lodding"
            v-if="loadData ? item.isLeaves : item.children"
          ></g-icon>
        </template>
        <template v-else>
          <g-icon
            class="down-icon"
            icon="right"
            v-if="loadData ? item.isLeaves : (item.children && item.children.length>0)"
          ></g-icon>
        </template>
      </div>
    </div>
...
export default {
  ...
  methods: {
    clickSelected(item, index) {
      ...
      obj[this.level] = Object.assign(
        {},
        { id: item.id, name: item.name, next }
      );
      //把当前 选中的下一层元素全部删除掉
